...
코인 탭은 BTC, ETH, XRP, XLM만 정상 운용 감시군으로 두고, 나머지 급등 코인은 예외 풀에서 별도로 추적합니다.

- 정상 감시군 비중 단계: 선발대 5%, 기초 30%, 기초+10 40%, 기초+20 50%, 기본 65%, 기본+17 82%, 맥스 100%
- 비트코인은 30분봉 단기 대응에 큰 비중을 싣지 않고, 주봉 20/30주선과 일봉 30일선을 먼저 봅니다.
- 주봉이 30주선 아래이면 대추세가 완전히 돌아선 것으로 보지 않아 맥스베팅을 피하고, 최대 기본물량 65%까지만 허용합니다.
- BTC 주봉 20주선 아래 + 일봉 30일선 위: 바닥 반등 에너지는 인정하되 기초+20, 즉 50%까지만 유지합니다.
- BTC 주봉 20주선 아래 + 일봉 30일선 아래: 상승 전환 확정 전이므로 기초 30% 또는 기초+10 40%까지만 허용합니다.
- BTC 주봉 20주선 위 + 일봉 30일선 위: 기본물량 65%까지 채우고, 주봉이 30주선 위에 서면 맥스 100%까지 열어둡니다.
- BTC 주봉 20주선 위 + 일봉 30일선 아래: 상방 전환 실패 또는 횡보 전환 가능성을 보고 기초~기초+20 범위로 관리합니다.
- BTC 일봉 300일 고점/저점 피보나치는 고점을 0%, 저점을 100%로 두고 현재 위치를 계산합니다. 61.8~100% 바닥권에서는 큰그림이 아직 정배열 전환 전이므로 기초물량 중심으로 봅니다.
- BTC가 최근 200일 저점을 하향 이탈하면 기존 목표비중의 60%를 덜고 40%만 남깁니다. 예를 들어 기본 65% 상태라면 26%가 되며, 기초물량 이하 꼬리표를 붙입니다.
- BTC 최근 60일 저점 대비 5% 이상 반등하면 손절 이후 회복 신호로 보고 기초 또는 기초+10 40% 셋업을 허용합니다.
- 예외 코인: 기본 풀 5%, 종목당 2.5%, 주봉 늦은 진입은 전체 1% 탐색과 30% 이상 넓은 손절
- 예외 풀 초과: 6~10%는 사용자 승인, 응답 없음은 +1% 임시 허용, 절대 15% 초과 금지
- 30분 리스크: 전환선, 기준선, 30이평, 피보 50%, 61.8% 순서로 경고와 축소를 분리
- 고점 대비 강제 축소: 메이저 15%, 알트 20%, 예외 25%를 감시하고, 해당 기준의 10% 초과 터치는 확정봉 없이 축소
- 재진입: 손절선 회복이나 최근 저점 대비 메이저 5%, 알트 7%, 예외 10% 반등을 확인하되 기본물량으로 바로 복귀하지 않고 기초+10 40%까지만 표시
- 기초에서 기본으로 늘릴 때: 추가 35% 중 15% 시장가, 20%는 현재가 2~7% 아래 또는 구름대 근처 지정가
